...
async def detect_batch_images(
    ...
    """
    批量图片检测接口（更健壮）：
    - 支持 files 和 files[] 两种字段名
    - 如果绑定为空，则从原始表单中解析
    """
    try:
        os.makedirs(settings.upload_dir, exist_ok=True)

        # 收集所有上传的文件（支持 files 和 files[]）
        parsed_files = []
        ...
        if files and isinstance(files, list):
            ...
            parsed_files.append(files)
        
        # 再添加 files[] 字段的文件
        if files_list and isinstance(files_list, list):
            parsed_files.extend(files_list)
        elif files_list:
        ...
        if not parsed_files:
            ...
            # 从 files[] 字段获取
            files_from_form = form.getlist("files[]") if hasattr(form, 'getlist') else []
            for v in files_from_form:
                if isinstance(v, UploadFile) and v not in parsed_files:
                    parsed_files.append(v)
            # 从 files 字段获取
            files_files = form.getlist("files") if hasattr(form, 'getlist') else []
            for v in files_files:
                if isinstance(v, UploadFile) and v not in parsed_files:
                    parsed_files.append(v)
            # 获取其他字段
            if not model_name:
...
async def get_detection_by_id(
    ...
    try:
        ...
        detection_result = DetectionResult(
            ...
            created_at=record.created_at,
            # 新增字段
            filename=original_key.split('/')[-1] if original_key else "",
            type=record.type or "single",
            status=record.status or "completed",
...
async def get_target_list():
    ...
    targets = [
        ...
        TargetItem(id=4, name="Raw_Litchi", chinese_name="未成熟荔枝", description="处于未成熟阶段的荔枝，果皮呈绿色或黄绿色"),
        TargetItem(id=5, name="Ripe_Litchi", chinese_name="成熟荔枝", description="已经成熟的荔枝，果皮呈红色或紫红色"),
    ]
...
async def download_detection_result(
    detection_id: str = Path(..., description="检测记录 ID"),
    type: str = "result"  # result: 检测结果图片, original: 原始图片
):
    """
    下载检测结果文件接口

    功能：
    - 根据检测 ID 下载检测结果图片或原始图片
    - 支持下载单图检测和视频检测的结果

    参数：
        detection_id: 检测记录 ID
        type: 下载类型（result: 检测结果图片, original: 原始图片）
    ...
    try:
        ...
        if type == "original":
            # 下载原始图片/视频
            bucket = _bucket_for_key(settings.minio.original_bucket, original_key)
            object_name = _object_name_from_key(original_key)
            fallback_url = _file_url(settings.minio.original_bucket, original_key)
        else:
            # 下载结果图片/视频
            if (record.type or "") == "video":
                # 视频检测返回处理后的视频
                video_key = media.get("video", "")
                if video_key:
                    bucket = _bucket_for_key(settings.minio.results_bucket, video_key)
                    object_name = _object_name_from_key(video_key)
                else:
                    # 回退到封面图
                    cover_key = media.get("cover", "")
                    bucket = _bucket_for_key(settings.minio.results_bucket, cover_key)
                    object_name = _object_name_from_key(cover_key)
            else:
                # 图片检测返回结果图片
                cover_key = media.get("cover") or media.get("fallback", "")
                bucket = _bucket_for_key(settings.minio.results_bucket, cover_key)
                object_name = _object_name_from_key(cover_key)
        ...
        print(f"[下载错误] Detection ID: {detection_id}, Type: {type}")
