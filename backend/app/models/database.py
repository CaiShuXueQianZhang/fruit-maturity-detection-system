...
try:
    ...
    # 提供最小回退实现：当环境无法安装 SQLAlchemy 时，使用简单占位符与内存会话
    def create_engine(url, *args, **kwargs):
        print("[WARNING] SQLAlchemy 未安装，数据库操作已禁用（回退模式）")
        return None
    ...
    # 简单类型占位符
    String = Integer = Float = DateTime = Text = Boolean = ForeignKey = lambda *a, **k: None

    def declarative_base():
        class DummyBase:
            metadata = type("M", (), {"create_all": staticmethod(lambda bind=None: None)})
...
# 尝试创建数据库引擎并验证连接；若失败则回退到本地 SQLite（便于开发调试）
engine = None
try:
    ...
    try:
        ...
        # 回退到 SQLite 本地文件
        sqlite_url = "sqlite:///./dev_db.sqlite3"
        engine = create_engine(sqlite_url, connect_args={"check_same_thread": False})
except Exception:
    # 无法构建 Postgres 引擎，使用 SQLite 回退
    sqlite_url = "sqlite:///./dev_db.sqlite3"
    engine = create_engine(sqlite_url, connect_args={"check_same_thread": False})
