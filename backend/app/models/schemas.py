...
class DetectionBox(BaseModel):
    ...
    # 目标类别名称（英文，如 "Raw_Banana", "Raw_Mango", "Ripe_Banana", "Ripe_Mango"）
    class_name: str
...
class DetectionResult(BaseModel):
    ...
    # 视频检测结果的视频 URL（仅视频检测时返回）
    video_url: Optional[str] = None

    # 视频检测结果的封面图 URL（仅视频检测时返回）
    ...
    # 检测类型（single/batch/folder/video）
...
class HistoryItem(BaseModel):
    ...
    # 视频检测结果的视频 URL（仅视频检测历史记录使用）
    video_url: Optional[str] = None

    # 视频检测结果的封面图 URL（仅视频检测历史记录使用）
    cover_image_url: Optional[str] = None
