...
#
# Bucket 说明：
#   - mab-original: 存储用户上传的原始图片
#   - mab-results: 存储检测后的结果图片
#   - mab-models: 存储 AI 模型文件（私有）
#
# 使用示例：
#   from app.services.minio_service import minio_service
#
#   # 上传图片
#   object_name = minio_service.upload_image(file, "mab-original")
#
#   # 获取访问 URL
#   url = minio_service.get_public_url("mab-original", object_name)
# =============================================================================
...
try:
    ...
    class _DummyClient:
        """本地回退客户端：将对象写入本地目录（settings.static_dir/minio/<bucket>）
        提供与 MinIO SDK 兼容的最小接口供上层调用。
        """
        def __init__(self, endpoint=None, access_key=None, secret_key=None, secure=False):
            self.base_dir = os.path.join(settings.static_dir or 'static', 'minio')
            os.makedirs(self.base_dir, exist_ok=True)

        def bucket_exists(self, bucket):
            return os.path.isdir(os.path.join(self.base_dir, bucket))

        def make_bucket(self, bucket):
            os.makedirs(os.path.join(self.base_dir, bucket), exist_ok=True)

        def set_bucket_policy(self, bucket_name, policy_str):
            # 本地回退不需要策略
            return None
        ...
        def put_object(self, bucket_name, object_name, data, length=None, content_type=None):
            ...
            out_path = os.path.join(bucket_dir, object_name)
            # data 可能是 BytesIO 或 bytes
            try:
                # 尝试从 file-like 对象读取
        ...
        def presigned_get_object(self, bucket_name, object_name, expires=3600):
            # 返回可由后端代理访问的本地 API 路径
            return f"/api/detection/files/{bucket_name}/{object_name}"

        def remove_object(self, bucket_name, object_name):
            try:
                os.remove(os.path.join(self.base_dir, bucket_name, object_name))
                return True
            except Exception:
                return False

        def list_objects(self, bucket_name, prefix=''):
            p = os.path.join(self.base_dir, bucket_name)
            if not os.path.isdir(p):
                return []
            class _Obj:
                def __init__(self, name):
                    self.object_name = name
            return [ _Obj(n) for n in os.listdir(p) if n.startswith(prefix) ]

        def get_object(self, bucket_name, object_name):
            path = os.path.join(self.base_dir, bucket_name, object_name)
            if not os.path.exists(path):
                raise FileNotFoundError(path)
            return _DummyObject(path)

    # 将 Minio 名称绑定到回退实现，便于后续代码统一使用
    Minio = _DummyClient
...
class MinIOService:
    ...
    def upload_file_bytes(self, bucket_name: str, file_bytes: bytes, object_name: str = None, content_type: str = None) -> str:
        """
        通用字节上传方法
        ...
        参数：
            bucket_name: 目标 Bucket 名称
            file_bytes: 文件字节数据
            object_name: 可选的对象名称（含扩展名），不提供则自动生成
        ...
        # 使用指定的 content_type 或默认的 application/octet-stream
        self.client.put_object(
            bucket_name=bucket_name,
            object_name=object_name,
            data=file_bytes_io,
    ...
    def upload_result_video(self, video_bytes: bytes, extension: str = "mp4") -> str:
        """
        上传检测后的视频结果到结果 Bucket
        """
        object_name = f"result_{uuid.uuid4().hex}.{extension}"
        file_bytes = io.BytesIO(video_bytes)
        self.client.put_object(
    ...
    def upload_video_bytes(self, video_bytes: bytes, original_filename: str) -> str:
        """
        上传原始视频字节到原始 Bucket
        """
        ...
        object_name = f"{uuid.uuid4().hex}.{file_extension}"
        file_bytes = io.BytesIO(video_bytes)
        content_type = f"video/{file_extension}" if file_extension in ["mp4", "avi", "mov", "mkv"] else "application/octet-stream"
        self.client.put_object(
            bucket_name=settings.minio.original_bucket,
            object_name=object_name,
            data=file_bytes,
    ...
    def get_public_url(self, bucket_name: str, object_name: str) -> str:
        ...
        功能：
            - 直接生成公开可访问的 URL
            - 适用于设置为公开访问的 Bucket（如 mab-original, mab-results）
    ...
    def get_latest_model(self, model_prefix: str = "mab-yolo11m-best") -> Optional[str]:
        """
        获取最新版本的模型（兼容新旧两种格式）
        ...
        try:
            ...
            def parse_model_name(filename: str):
                try:
                    # 新格式：mab-yolo11m-best_v1.0.0_20240101090000.pt
                    if '_v' in filename:
                    ...
                    # 旧格式：mab-yolo11m-best_1779125662.pt
    ...
    def get_model_metadata(self, model_object_name: str) -> Optional[dict]:
        ...
        参数：
            model_object_name: 模型对象名称（如 mab-yolo11m-best_v1.0.0_20240101090000.pt）
    ...
    def list_models_with_metadata(self, model_prefix: str = "mab-yolo11m-best") -> list:
        """
        列出所有模型及其元数据
