...
from app.api.debug_upload import router as debug_router         # 调试上传路由
from app.api.auth import router as auth_router                 # 认证路由
from app.api.qa import router as qa_router                       # AI 问答 API 路由
...
# 尝试初始化数据库（在无法连接 Postgres 时会回退到本地 SQLite）
from app.models.database import init_db
try:
    init_db()
except Exception as e:
    print(f"[WARN] 初始化数据库失败: {e}")
