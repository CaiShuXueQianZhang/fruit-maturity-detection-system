    ...
    <div class="main-content">
      ...
      <div class="left-panel">
        <div class="panel-header">
          <span class="panel-title">{{ activeTab === 'ciname' ? '摄像头实时检测' : '检测预览' }}</span>
          <el-tag :type="activeTab === 'ciname' ? (isCameraRunning ? 'success' : 'info') : (hasImage && detectionResult ? 'success' : 'info')" effect="light">
            <el-icon v-if="activeTab === 'ciname'"><VideoCamera /></el-icon>
            <el-icon v-else-if="hasImage && detectionResult"><Check /></el-icon>
            <el-icon v-else><Upload /></el-icon>
            {{ activeTab === 'ciname' ? (isCameraRunning ? '检测中' : '未启动') : (hasImage && detectionResult ? '检测完成' : '等待上传') }}
          </el-tag>
        </div>

        <!-- 摄像头模式显示组件 -->
        <component v-if="activeTab === 'ciname'" :is="currentComponent" @update:running="isCameraRunning = $event" />

        <!-- 单图/批量模式显示原有内容 -->
        <template v-else>
          <div class="toolbar">
            <el-button :class="{ active: compareMode === 'side' }" size="small" @click="compareMode = 'side'"><el-icon><Minus /></el-icon>并排对比</el-button>
            <el-button :class="{ active: compareMode === 'grid' }" size="small" @click="compareMode = 'grid'"><el-icon><Grid /></el-icon>栅格对比</el-button>
          </div>
          <div class="image-compare">
            <div class="image-card">
              ...
              <template v-else>
                <div class="image-placeholder"><el-icon class="placeholder-icon"><Upload /></el-icon><p class="placeholder-text">请上传图片</p><p class="placeholder-desc">支持 jpg、png 格式</p></div>
              </template>
            ...
            <div class="image-card">
              ...
              <template v-else>
                <div class="image-placeholder"><el-icon class="placeholder-icon"><View /></el-icon><p class="placeholder-text">检测结果将在此展示</p><p class="placeholder-desc">上传图片后开始检测</p></div>
              </template>
...
const currentComponent = CameraDetection;
...
const handleTabClick = (key) => {
  activeTab.value = key;
  if (key === 'ciname') {
    // 摄像头模式：不触发文件选择，可以在这里自动启动摄像头，或者由用户点击组件内的按钮
    // 由于组件内部有“开启摄像头”按钮，所以这里什么都不用做
    return;
  }
